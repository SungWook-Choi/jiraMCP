...
6. 같은 작업지시서 안에서 이어지는 재작업에만 기존 `sub-agent` 재사용을 허용한다.
7. 작업 항목이 바뀌면 기존 `sub-agent`를 재사용하지 않는다.
8. 작업지시서가 없거나 불완전하면 작업 할당과 검수를 모두 보류한다.
9. `sub-agent`가 완료 보고를 할 때까지 기다린다.
10. 결과물은 `AGENTS.md`, 체크리스트, 작업지시서를 기준으로 검수한다.
11. 설명보다 실제 변경 결과와 검증 근거를 더 신뢰한다.
12. 기준 미달이면 패스하지 않고 재작업을 요청한다.
13. 통과할 때까지 재작업을 반복한다.
14. 같은 작업 항목에서 같은 유형의 재작업이 2회 이상이면 사용자 승인 전제의 강제 규칙 초안을 만든다.
15. 재작업 요청에는 실패 사유, 근거, 수정 요구사항을 반드시 포함한다.
16. 체크리스트가 비어 있으면 검수를 보류한다.
